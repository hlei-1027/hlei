...
ya_modified = zeros(ny, N);    % 主动修改数据后的ya
ya_modified(:,1) = omiga_k(:,:,1) * ya(:,1);

ya_restored = zeros(ny, N);    % 主动恢复数据后ya'
ya_restored(:,1) = inv(omiga_k(:,:,1)) * ya_(:,1);  % 需要订正

ua_modified = zeros(ny, N);    % 主动修改后的ua
ua_modified(:,1) = psai_k(:,:,1) * ua(:,1);

ua_restored = zeros(ny, N);    % 主动恢复后的ua'
